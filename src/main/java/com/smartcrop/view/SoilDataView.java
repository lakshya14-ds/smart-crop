...
public class SoilDataView {
    ...
    public SoilDataView(String userEmail, Consumer<SoilData> onSave) {
        this(userEmail, SupabaseService.getInstance(), onSave);
    }

    public SoilDataView(String userEmail, SupabaseService supabase, Consumer<SoilData> onSave) {
        root = new VBox(20);
        ...
        ColumnConstraints nc1 = new ColumnConstraints();
        nc1.setPercentWidth(33.3);
        ColumnConstraints nc2 = new ColumnConstraints();
        nc2.setPercentWidth(33.3);
        ColumnConstraints nc3 = new ColumnConstraints();
        nc3.setPercentWidth(33.3);
        ...
        saveBtn.setOnAction(e -> {
            ...
            new Thread(() -> {
                try {
                    ...
                    System.out.println("🚀 Sending data to Supabase...");

                    String res = db.saveSoilData(
                            "lakshya123cms@gmail.com",
                            data.getSoilType(),
                            data.getPh(),
                            data.getNitrogen(),
                            data.getPhosphorus(),
                            data.getPotassium(),
                            data.getOrganicMatter(),
                            data.getMoisture());
                    ...
                    javafx.application.Platform.runLater(() -> {
                        ...
                        alert.setContentText("✅ Soil Data Saved!");
                        alert.show();
                    });
                    ...
                    javafx.application.Platform.runLater(() -> {
                        ...
                        alert.setContentText("❌ Failed to save: " + ex.getMessage());
                        alert.show();
                    });
                }
            }).start();
        ...
        root.getChildren().addAll(title, soilTypeBox, phMoistureGrid,
                nutrientHeader, nutrientGrid, organicBox, statusLabel, saveBtn);
    }

    private Spinner<Double> makeDoubleSpinner(double min, double max, double init, double step) {
        SpinnerValueFactory.DoubleSpinnerValueFactory factory = new SpinnerValueFactory.DoubleSpinnerValueFactory(min,
                max, init, step);
